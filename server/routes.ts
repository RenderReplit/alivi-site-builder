...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const handleMulterError = (err: any, req: Request, res: Response, next: NextFunction) => {
    if (err) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        res.status(400);
        return res.json({ success: false, error: "Arquivo muito grande. Tamanho máximo: 50MB" });
      }
      if (err.message === 'Invalid file type') {
        res.status(400);
        return res.json({ success: false, error: "Tipo de arquivo inválido" });
      }
      res.status(400);
      return res.json({ success: false, error: err.message || "Erro ao processar arquivo" });
    }
    next();
  };

  // File upload routes
  app.post("/api/upload/smile-photo", upload.single("smilePhoto"), handleMulterError, (req: Request, res: Response) => {
    try {
      if (!req.file) {
  ...
  app.post("/api/upload/scanner-file", upload.single("scannerFile"), handleMulterError, (req: Request, res: Response) => {
    try {
      if (!req.file) {
        return res.status(400).json({ success: false, error: "Nenhum arquivo foi enviado" });
      }
